...
function updateTimer() {
    const interval = setInterval(function() {
        ...
        timerDiv.innerHTML = `<span style="color: #fff500;">Next update at:</span> <span style="color: #fff500; text-decoration: 1px white underline;">${hours.toString().padStart(2, '0')}h ${minutes.toString().padStart(2, '0')}m ${seconds.toString().padStart(2, '0')}s</span>`;

        if (remainingTime <= 0) {
            clearInterval(interval);
            timerDiv.innerHTML = `<span style="color: #50e3c2;">Updates are live!</span>`;
